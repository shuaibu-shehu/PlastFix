...
import Newsfeed from '../caresoul/newfeed';
...
export default function DashboardPage({user, items}: {user: any; items: any}) {
  ...
  const compareLastTwoMonths = (type) => {
    ...
    const currentMonthWeight = currentMonth.reduce(
      (total, item) => total + item.weight,
      0
    );
    const lastMonthWeight = lastMonth.reduce(
      (total, item) => total + item.weight,
      0
    );
  ...
  const calculatePlasticStats = (items) => {
    ...
    const nonRecyclableCompareLastMonth =
      compareLastTwoMonths('non-recyclable');
    ...
    const recyclableWeight = recyclableItems.reduce(
      (total, item) => total + item.weight,
      0
    );

    const nonRecyclableItems = items.filter(
      (item) => item.type === 'non-recyclable'
    );
    const nonRecyclableWeight = nonRecyclableItems.reduce(
      (total, item) => total + item.weight,
      0
    );

    const singleUseItems = items.filter((item) => item.type === 'single-use');
    const singleUseWeight = singleUseItems.reduce(
      (total, item) => total + item.weight,
      0
    );

    const recyclablePercentage =
      totalWeight > 0 ? (recyclableWeight / totalWeight) * 100 : 0;
    const nonRecyclablePercentage =
      totalWeight > 0 ? (nonRecyclableWeight / totalWeight) * 100 : 0;
    const singleUsePercentage =
      totalWeight > 0 ? (singleUseWeight / totalWeight) * 100 : 0;
    ...
    <div className='space-y-6'>
      <CardHeader className='bg-green-600 text-white rounded-t-lg'>
        <CardTitle className='text-2xl font-bold'>
          <span className='text-3xl'>
            Hi <span>{user?.name?.toLocaleUpperCase()}</span> 🖐
          </span>{' '}
          <br />
      ...
      <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-4'>
        ...
        <Card>
          ...
          <CardContent>
            <div className='text-2xl font-bold text-green-600'>
              {recyclableCompareLastMonth.weight
                ? recyclableCompareLastMonth.weight.toFixed(2)
                : '0.00'}{' '}
              g
            </div>
            <p className='text-xs text-muted-foreground'>
              {recyclableCompareLastMonth.status == 'up'
                ? `+${recyclableCompareLastMonth.percentage}`
                : `-${recyclableCompareLastMonth.percentage}`}
              % from last month
            </p>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <div className='text-2xl font-bold text-green-600'>
              {recyclableWeight ? recyclableWeight : '0.00'} g
            </div>
            <p className='text-xs text-muted-foreground'>
              {recyclablePercentage}% of total waste
            </p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className='flex flex-row items-center justify-between space-y-0 pb-2'>
            <CardTitle className='text-sm font-medium'>
              Non-Recyclables items
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className='text-2xl font-bold text-[#dc2626]'>
              {nonRecyclableWeight ? nonRecyclableWeight : '0.00'} g
            </div>
            <p className='text-xs text-muted-foreground'>
              {nonRecyclablePercentage}% of total waste
            </p>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent>
            <div className='text-2xl font-bold text-orange-600'>
              {singleUseCompareLastMonth.weight
                ? singleUseCompareLastMonth.weight.toFixed(2)
                : '0.00'}{' '}
              g
            </div>
            <p className='text-xs text-muted-foreground'>
              <span className='text-xs text-muted-foreground'>
                {singleUseCompareLastMonth.status == 'up'
                  ? `+${singleUseCompareLastMonth.percentage}`
                  : `-${singleUseCompareLastMonth.percentage}`}
                % from last month
              </span>
            </p>
